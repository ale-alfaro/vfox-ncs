...
M.platform_idents = function()
    ...
    local osname = os_map[RUNTIME.osType:lower()] or RUNTIME.osType:lower()
    local arch = arch_map[RUNTIME.archType:lower()] or RUNTIME.archType:lower()
    local extensions = { archive = get_platform_archive_suffix(), executable = get_platform_exe_suffix() }
    return { osname = osname, arch = arch, ext = extensions }
end
